...
Page({
  data: {
    ...
    scoringMode: 'alternate', stamina: 100, isDunkLayup: false, commentary: [],
  },
  ...
  onUnload: function() {
    if (this._animId) this._canvas.cancelAnimationFrame(this._animId)
    if (this._animId) { this._canvas.cancelAnimationFrame(this._animId); this._animId = null }
if (this._timer) clearInterval(this._timer)
  },

  // ============ 本地模式 (requestAnimationFrame 丝滑渲染) ============
  _startLoop: function() {
    var that = this
    this._last = Date.now()
    this._frameCount = 0
    var loop = function() {
      if (!that._engine || that._ended) return
      that._animId = that._canvas.requestAnimationFrame(loop)
      var now = Date.now(), dt = now - that._last
      that._last = now
      if (dt > 100) dt = 33 // 防大跳帧
      that._engine.update(dt)
      that._engine.render()
      // 每3帧同步一次UI (减少setData调用)
      that._frameCount++
      if (that._frameCount % 3 === 0) that._syncUI()
    }
  ...
  _syncUI: function() {
    ...
    this.setData({
      ...
      shooting: s.shooting,
      isDunkLayup: s.isDunkLayup,
      commentary: s.commentary,
      ballLoose: s.ballLoose,
      msg: s.messageText,
      stamina: s.stamina,
    })
    if (s.impact > 0 && this._lastImpact <= 0) {
      if (s.impact >= 400) { wx.vibrateShort({ type: 'heavy' }); setTimeout(function() { wx.vibrateShort({ type: 'heavy' }) }, 150) }
      else if (s.impact >= 200) { wx.vibrateShort({ type: 'medium' }); setTimeout(function() { wx.vibrateShort({ type: 'medium' }) }, 120) }
      else wx.vibrateShort({ type: 'light' })
    }
    this._lastImpact = s.impact
  ...
  onShootDown: function(e) {
    if (this._engine && this._engine._jumpBallPhase !== 'done') {
      this._engine.jumpBallJump()
      return
    }
